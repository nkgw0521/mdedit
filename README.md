...
- タブはドラッグ&ドロップで並び替えられます
- Ctrl+Tab / Ctrl+Shift+Tab で次/前のタブへ切り替えられます

閉じたときに開いていたタブ(保存していない内容も含む)は記憶され、次回起動時に自動的に復元されます。

### ファイル操作

「File」メニュー、またはショートカットキーで操作できます。
...
「Edit」メニュー、またはCtrl+Z(元に戻す)/ Ctrl+Y・Ctrl+Shift+Z(やり直す)で操作できます。1文字ごとではなく、入力の切れ目(0.5秒程度の間が空いたタイミング)ごとに1つ戻る単位でまとまります。Undo/Redoの履歴はタブごとに独立しています。
...
**表示サイズの指定**: altテキストの末尾に`|幅`または`|幅x高さ`を書くと、画像の表示サイズを指定できます。

```markdown
![|300](screenshot.assets/paste-xxx.png)
![説明文|300x200](screenshot.assets/paste-xxx.png)
```

これはこのアプリ独自の記法で、標準のMarkdownには無いものです。他のMarkdownツールで開くと`|300`の部分がそのままalt文字列として表示されます(動作は壊れませんが、見た目に残ります)。

### 表の貼り付け(Excel等から)

Excelで選択したセル範囲をコピーしてエディタに貼り付けると、自動的にMarkdownの表(`| A | B |` 形式)に変換されます。WebページやWordなど、HTML形式の表データをクリップボードに含む他のアプリからの貼り付けでも同様に動きます。タブ区切りのテキストしか無い場合(text/htmlが無い環境)でも、タブを手がかりに表として認識します。

1列だけのデータや、表と判断できない内容は、通常のテキストとしてそのまま貼り付けられます。
...
Explorer(Windows)やファイルマネージャー(Linux)で`.md`ファイルをこのアプリに関連付けている場合、ダブルクリックするとそのファイルが開きます。既にアプリを起動している状態で別の`.md`ファイルをダブルクリックした場合も、新しいウィンドウは開かず、既存のウィンドウに新しいタブとして追加されます。

アプリを関連付けるには:

- **Windows**: エクスプローラーで`.md`ファイルを右クリック→「プログラムから開く」→「別のプログラムを選択」から、このアプリの実行ファイルを選択してください。
- **Linux**: ディストリビューションのファイルマネージャーの「アプリケーションで開く」機能から設定してください。環境によっては、あらかじめ`.desktop`ファイルを用意して`xdg-mime`で関連付ける必要がある場合があります。
